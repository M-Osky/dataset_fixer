...
#######         CHANGE LOG                                                                                       ##########
#######                                                                                                          ##########
#######    2019-05-03  -  Version 3.1                                                                            ##########
#######     Changed output files to display missing replacer options                                             ##########
#######     Now genepop file extention is ".gen" as required by Adegenet                                         ##########
#######                                                                                                          ##########
...
\t\t                            A log file from PGDSpider, $VERSION, and from each module will be saved at the working directory.
		  ...
		  --endtail                 string to add at the end of the input file to generate the final output file name (default: $SUFIX6).
		  --custom_out              (yes/no) Should $VERSION add a code to the endtail to indicate the options chosen in missing_replacer?
		                            two characters: first = miss_gral option (1g=global, 1p=pop, 0g=leave it as missing)
		                                           second = miss_pop  option (2g=global, 5p=005005, 0p=leave it as missing)
		                            final end tail if \"yes\": filename_FINAL1p5p.str; filename_FINAL1g0p.str; etc. (default: $CUSTOMOUT).\n\n
...
then
	...
	SUFIX6="$SUFIX6$GLOBCHOICE$POPCHOICE"
fi
